...
MESSAGE_1="\n X11VNC-server screen resolution? \
           ...
           \n unchanged is selected via timeout. \
           \n \
           \n"
...
case $ACTION in
   ...
   3)  # 1024x768 was selected
       ...
       ;;
   70) # unchanged was selected via timeout 
...
if [ "$DESKTOP_SIZE_MODE" = "1024x768" ]; then
   ...
   X11VNC_GEOMETRY="-bg -geometry 1024x768"
fi

if [ "$DESKTOP_SIZE_MODE" = "1360x768" ]; then

   X11VNC_GEOMETRY="-bg -geometry 1360x768"
fi

if [ "$DESKTOP_SIZE_MODE" = "unchanged" ]; then
